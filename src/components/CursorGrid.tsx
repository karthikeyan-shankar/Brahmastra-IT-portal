...
export const CursorGrid = ({
  cellSize = 70,
  color = '#06b6d4', // Cyan color to match the tech theme
  radius = 140,
  falloff = 'smooth',
  holdTime = 400,
  fadeDuration = 800,
  ...
  useEffect(() => {
    ...
    const energize = (x: number, y: number, boost?: number) => {
      const p = propsRef.current;
      if (!p.radius || !p.falloff || !p.cellSize || !p.maxOpacity) return;
    ...
    const draw = (now: number) => {
      const p = propsRef.current;
      if (!p.color || !p.cellSize || p.gridOpacity === undefined || p.fadeDuration === undefined || p.holdTime === undefined) return;
      ...
      for (let i = 0; i < alphas.length; i++) {
        ...
        ctx.beginPath();
        if (p.cellRadius && p.cellRadius > 0) {
          ctx.roundRect(x, y, s, s, p.cellRadius);
        } else {
          ctx.rect(x, y, s, s);
        }
        if (p.fillOpacity && p.fillOpacity > 0) {
    ...
    // Attach to window so it tracks globally even if container is behind things
  ...
  return (
    <div ref={containerRef} className={`absolute inset-0 overflow-hidden pointer-events-none -z-15 ${className}`}>
